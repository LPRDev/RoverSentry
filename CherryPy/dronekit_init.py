...
class DroneControl(object):
	...
	def waypoints(self):
		...
		msg+= "Next Waypoint location is Waypoint %s located at lat=%s lon=%s, Alt=%s <BR>" % (cmds.next , cmds[cmds.next-1].x, cmds[cmds.next-1].y, cmds[cmds.next-1].z)
		return msg
	###########################################################################
